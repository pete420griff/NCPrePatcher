...
module Build
  ...
  def self.run(args)
    if !Dir.exist?(NITRO_BUILD_PATH) || args.include?('--config')
      config_nitro
    end

    if args.include? 'nitro'
      build_nitro
      return
    end

    if args.include? 'unarm'
      build_unarm
      return
    end

    if args.include? 'unicorn'
      build_vcpkg_lib('unicorn')
      return
    end

    if args.include? 'keystone'
