  ...
  class Interpreter
    def initialize(cmd_prefix: @@COMMAND_PREFIX, extra_cmds: {}, extra_vars: {})
      ...
      @commands = {
        ...
        define_command: ->(name,block) { Utils::valid_identifier_check(name); @commands[name.to_sym] = (block.is_a?(Block) ? block : eval(block, get_binding) },
        alias_command: ->(new_name,name) { Utils::valid_identifier_check(name);  @commands[new_name.to_sym] = @commands[name.to_sym] },
        define_variable: ->(name, val) { Utils::valid_identifier_check(name); @variables[name.to_sym] = eval_expr(val)},
        alias_variable: ->(new_name,name) { Utils::valid_identifier_check(name); @variables[new_name.to_sym] = @variables[name.to_sym] },
    ...
    Block = Struct.new(:ast) do
      def eval()
        result = nil
        ast.each { result = eval_expr(it) }
        result
      end
    ...
    def process_file(file_path, verbose: true)
      ...
      File.readlines(file_path).each_with_index do |line, lineno|
        ...
        while cursor < line.length
          # stop parsing rest of line on // outside comment/string
          ...
          # check for commands
          if !in_comment && !in_string && line[cursor, $COMMAND_PREFIX.length] == $COMMAND_PREFIX && (cursor == 0 || !/[0-9A-Za-z_]/.match?(line[cursor-1]))
            expr_src = line[(cursor + $COMMAND_PREFIX.length)..]

            begin
              parser      = NCPP::Parser.new
              transformer = NCPP::Transformer.new
              tree        = parser.parse(expr_src)
              rtree_s     = tree.to_s.reverse

              # EXTREMELY HACKY BUT FOR NOW FUCK IT I DON'T CARE I DON'T CARE I DON'T CARE
              last_paren = Integer(/\d+/.match(rtree_s[..rtree_s.index('__outer_paren__: '.reverse)].reverse).to_s) + 1

              ast = transformer.apply(tree)
              value = NCPP.eval_command(ast)
              new_line << value.to_s

              puts "#{file_path}:#{lineno+1} expanded #{$COMMAND_PREFIX}..." if verbose
              cursor += $COMMAND_PREFIX.length + last_paren
              next
            rescue Parslet::ParseFailed => e
              warn "#{file_path}:#{lineno+1}: parse failed at expr"
